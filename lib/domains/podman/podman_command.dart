...
class PodmanCommand extends Command {
  ...
  @override
  final description = 'Build, manage, or view logs of the Podman container for the project.';
...
''';
  ...
  PodmanCommand() {
    argParser.addOption(
      'image',
      abbr: 'i',
      help: 'Name of the Podman image to build/run. You will be prompted if not provided.',
    );
    argParser.addFlag(
      'logs',
      abbr: 'l',
  ...
  Future<void> run() async {
    ...
    if (args.isNotEmpty) {
      ...
      if (subcommand == 'start') {
        final result = await Process.run('podman', ['machine', 'start', 'podman-machine-default']);
        stdout.write(result.stdout);
        stderr.write(result.stderr);
        exit(result.exitCode);
      } else if (subcommand == 'stop') {
        final result = await Process.run('podman', ['machine', 'stop', 'podman-machine-default']);
        stdout.write(result.stdout);
        stderr.write(result.stderr);
        exit(result.exitCode);
      } else if (subcommand == 'reset') {
        // Check if podman-machine-default exists
        final lsResult = await Process.run('podman', ['machine', 'list', '--format', '{{.Name}}']);
        ...
        if (machineExists) {
          ...
          for (final step in steps) {
            ...
            if (result.exitCode != 0) {
              stderr.writeln('Error: command "${step.join(' ')}" failed with exit code ${result.exitCode}.');
              exit(result.exitCode);
            }
          }
        } else {
          stdout.writeln('Podman machine "podman-machine-default" does not exist. Initializing and starting a new machine.');
          final initResult = await Process.run('podman', ['machine', 'init', '--cpus', '4', '--memory', '4096', '--disk-size', '20']);
          stdout.write(initResult.stdout);
          stderr.write(initResult.stderr);
          if (initResult.exitCode != 0) {
            stderr.writeln('Error: command "podman machine init" failed with exit code ${initResult.exitCode}.');
            exit(initResult.exitCode);
          }

          final startResult = await Process.run('podman', ['machine', 'start', 'podman-machine-default']);
          stdout.write(startResult.stdout);
          stderr.write(startResult.stderr);
          if (startResult.exitCode != 0) {
            stderr.writeln('Error: command "podman machine start" failed with exit code ${startResult.exitCode}.');
          ...
          if (psResult.exitCode != 0) {
            stderr.writeln('Error: command "podman ps" failed with exit code ${psResult.exitCode}.');
            exit(psResult.exitCode);
          }
        }
        exit(0);
      } else if (subcommand == 'bind' && args.length > 1 && args[1].toLowerCase() == 'mount') {
    ...
    if (showLogs) {
      ...
      final result = await Process.run('podman', ['images', '--format', '{{.Repository}} {{.CreatedAt}}']);
      if (result.exitCode != 0) {
      ...
      lines.sort((a, b) {
        final dateA = DateTime.tryParse(a.split(' ').sublist(1).join(' ')) ?? DateTime(1970);
        final dateB = DateTime.tryParse(b.split(' ').sublist(1).join(' ')) ?? DateTime(1970);
      ...
      final psResult = await Process.run('podman', ['ps', '-a', '--filter', 'ancestor=$lastImage', '--format', '{{.ID}}']);
      if (psResult.exitCode != 0 || (psResult.stdout as String).trim().isEmpty) {
      ...
      // Show logs
      final logProcess = await Process.start('podman', ['logs', '-f', containerId]);
